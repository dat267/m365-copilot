...
| `src/prompt.js` | prompt assembly (`buildPrompt` context + system wrapping, `loadContext`, `resolveSystemPrompt`: `--system` → `SYSTEM.md` in the config dir → none) — no arg parsing, commander owns that |
